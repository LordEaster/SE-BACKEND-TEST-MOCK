**สวัสดีครับทุกท่าน วันนี้ผมจะมาอธิบายโจทย์ backend จำนวน 5 ข้อที่น่าสนใจ ทั้งการจัดการกับข้อมูลใน array การคำนวณผลลัพธ์ และการสลับหรือหมุนตำแหน่งสมาชิกใน array**

ลิงก์โจทย์: [โจทย์จำลอง back-end](https://github.com/LordEaster/SE-BACKEND-TEST-MOCK)

# วิธีการตรวจด้วย Automated

![](data:image/png;base64,)
ในไฟล์ที่เพื่อนๆ ดาวน์โหลดไปจะมีตัว checker อยู่
สิ่งที่ต้องทำ

1. ต้องมี python ติดตั้งอยู่ในเครื่อง
2. package python ที่ต้องติดตั้ง
    1. requests: สำหรับส่ง HTTP requests ไปยัง Piston API
    2. PyYAML: สำหรับอ่านและเขียนไฟล์ YAML
3. ติดตั้งได้ผ่าน `pip install requests pyyaml`
4. รันคำสั่ง `python3 checker/checker.py` ที่นอกสุดของโค้ดโจทย์ที่ดาวน์โหลดมา

**เราเข้าใจเบื้องต้นกันแล้วแล้ว ไปเริ่มกันเลยย!!**

### โจทย์ข้อที่ 1: บวกค่าตัวเลขจาก array โดยจัดการข้อมูลประเภท string ที่มีตัวเลขผสมอยู่

โจทย์นี้จะให้เรารับ array เป็น input และทำการบวกค่าทุก ๆ ตำแหน่งที่มีเลขแฝงอยู่ เงื่อนไขการเช็คคือ:

* ถ้าค่าที่ตำแหน่งนั้นเป็น int ให้ทำการบวกทันที
* ถ้าเป็น string ที่มีตัวเลขปนอยู่ ให้ดึงตัวเลขออกมาและบวกเข้าตัวแปร

**ตัวอย่าง**:

```array = [1, '2a', '3abc', 4, 'def5', 'b6'];```

ผลลัพธ์จะเท่ากับ 21 เพราะเราจะได้ตัวเลขจากการดึงออกมาจาก string แล้วบวกเข้ากับ int โดยตรง

**หลักการคิด**:

1. เราตรวจสอบแต่ละค่าของ array
2. ถ้าค่าเป็นตัวเลข ก็ทำการบวก
3. ถ้าค่าเป็น string ให้ดึงเฉพาะตัวเลขออกมาและเปลี่ยนเป็น int ก่อนที่จะทำการบวก

---

### โจทย์ข้อที่ 2: สลับค่าใน array

โจทย์นี้จะให้เราสลับค่าในตำแหน่งที่กำหนด โดยให้สลับค่าในตำแหน่ง index 1 กับ index 2 ใน array ที่ได้รับ

**ตัวอย่าง**:

`student = ['Bomgay', 'Ohm', 'Smart'];`

หลังจากการสลับ index ที่ 1 และ 2 ผลลัพธ์จะเป็น:

`student = ['Bomgay', 'Smart', 'Ohm'];`

**หลักการคิด**:

1. ตรวจสอบตำแหน่งของสมาชิกที่ต้องสลับ
2. ทำการสลับค่าในตำแหน่งที่กำหนดแล้ว return array ที่สลับค่าแล้วออกมา

---

### โจทย์ข้อที่ 3: เปรียบเทียบคะแนนของ Alice และ Bob

ในข้อนี้ Alice และ Bob จะทำการเปรียบเทียบคะแนนใน array ของแต่ละคน โดยเงื่อนไขคือ:

* ถ้า Alice ชนะในตำแหน่งนั้น ๆ Alice จะได้ 1 คะแนน
* ถ้า Bob ชนะในตำแหน่งนั้น ๆ Bob จะได้ 1 คะแนน
* ถ้าคะแนนเท่ากันจะไม่มีใครได้คะแนน

**ตัวอย่าง**:

`Alice = [1, 2, 3]; Bob = [7, 1, 3];`

จากตัวอย่าง Alice ได้ 1 คะแนน และ Bob ได้ 1 คะแนน ดังนั้นผลลัพธ์จะเป็น [1, 1]

**หลักการคิด**:

1. เปรียบเทียบแต่ละ index ของ Alice และ Bob
2. ให้คะแนนตามเงื่อนไขที่กำหนด
3. Return ผลรวมของคะแนนทั้งสองฝั่ง

---

### โจทย์ข้อที่ 4: หาผลรวมของค่ามากที่สุดและน้อยที่สุดใน array

ในโจทย์นี้ เราจะต้องหาค่ามากที่สุด (max) และค่าน้อยที่สุด (min) จาก array ที่ได้รับ จากนั้นให้นำค่าทั้งสองมาบวกกันแล้ว return ผลลัพธ์ออกมา

**ตัวอย่าง**:

`array = [3, 4, 5, 1, 2];`

ค่า max คือ 5 และค่า min คือ 1 ดังนั้นผลลัพธ์ของโจทย์นี้จะเท่ากับ 6

**หลักการคิด**:

1. หาค่าที่มากที่สุดและน้อยที่สุดใน array
2. นำค่ามากที่สุดและน้อยที่สุดมาบวกกัน
3. Return ผลลัพธ์ของการบวก

---

### โจทย์ข้อที่ 5: การหมุน (rotation) สมาชิกใน array

โจทย์นี้จะให้ทำการ “หมุน” สมาชิกใน array โดยย้ายสมาชิกตัวสุดท้ายไปไว้ตำแหน่งแรก และทำซ้ำตามจำนวนครั้งที่กำหนด

**ตัวอย่าง**:

`array = [1, 2, 3, 4, 5];`

หลังจากหมุนไป 3 ครั้ง ผลลัพธ์สุดท้ายจะเป็น:

`[3, 4, 5, 1, 2]`

**หลักการคิด**:

1. ย้ายสมาชิกตัวสุดท้ายไปไว้ที่ตำแหน่งแรกของ array
2. ทำซ้ำตามจำนวนครั้งที่ต้องการหมุน
3. Return array ที่ถูกหมุนเสร็จแล้ว

---

#### โจทย์ข้อที่ 6: หาเลขคู่ใน array
คำอธิบาย: 
ให้รับ array เป็น input แล้ว return array ที่มีเฉพาะเลขคู่เท่านั้น

**ตัวอย่าง**:
`array = [1, 2, 3, 4, 5, 6];`

ผลลัพธ์:
`[2, 4, 6]`

**หลักการคิด**:
1. ตรวจสอบแต่ละค่าของ array ว่าเป็นเลขคู่หรือไม่
2. ถ้าเป็นเลขคู่ให้นำมาเก็บใน array ใหม่
3. Return array ที่มีเฉพาะเลขคู่

---

#### โจทย์ข้อที่ 7: รวมข้อมูลใน array เป็น string เดียว
คำอธิบาย:
ให้รับค่า array ที่มีข้อมูลเป็น string และนำค่าทั้งหมดมารวมกันเป็น string เดียว แล้ว return ผลลัพธ์

**ตัวอย่าง**:
`array = ['Hello', 'World', 'from', 'Backend'];`

ผลลัพธ์:
`'Hello World from Backend'`

**หลักการคิด**:
1. รวมค่าใน array เข้าด้วยกันโดยใช้ช่องว่างเป็นตัวเชื่อม
2. Return string ที่รวมกันแล้ว

---

#### โจทย์ข้อที่ 8: ค้นหาค่าที่มีใน array หนึ่งแต่ไม่มีในอีก array หนึ่ง
คำอธิบาย:
ให้รับค่า array สองตัว แล้ว return ค่าใน array แรกที่ไม่มีอยู่ใน array ที่สอง

**ตัวอย่าง**:
`array1 = [1, 2, 3, 4, 5];`
`array2 = [2, 4, 6];`

ผลลัพธ์:
`[1, 3, 5]`

**หลักการคิด**:
1. ตรวจสอบค่าใน array1 ว่ามีใน array2 หรือไม่
2. ถ้าไม่มีใน array2 ให้นำมาเก็บใน array ใหม่
3. Return array ที่มีค่าเฉพาะใน array1

---

#### โจทย์ข้อที่ 9: การนับจำนวนการเกิดซ้ำใน array
คำอธิบาย:
ให้รับ array เป็น input และทำการนับจำนวนครั้งที่ค่าต่างๆ ปรากฏใน array นั้นๆ แล้ว return ผลลัพธ์เป็น object ที่เก็บค่าและจำนวนการเกิดซ้ำ

**ตัวอย่าง**:
`array = [1, 2, 2, 3, 3, 3, 4];`

ผลลัพธ์:
`{1: 1, 2: 2, 3: 3, 4: 1}`

**หลักการคิด**:
1. สร้าง object ที่เก็บค่าจำนวนครั้งที่แต่ละค่าปรากฏใน array
2. Return object ที่บันทึกจำนวนการเกิดซ้ำ

---

#### โจทย์ข้อที่ 10: เช็คว่าข้อมูลใน array ทั้งหมดเป็นเลขคู่หรือไม่
คำอธิบาย:
ให้รับ array เป็น input แล้ว return ค่า boolean ว่า array นั้นมีแต่เลขคู่หรือไม่

**ตัวอย่าง**:
`array = [2, 4, 6, 8];`

ผลลัพธ์:
`true`

**หลักการคิด**:
1. ตรวจสอบว่าแต่ละค่าของ array เป็นเลขคู่หรือไม่
2. ถ้าค่าใดค่าเดียวไม่ใช่เลขคู่ return false ทันที
3. ถ้าครบทุกค่าคือเลขคู่ return true

---

#### โจทย์ข้อที่ 11: หาผลคูณของค่าทั้งหมดใน array
คำอธิบาย:
ให้รับ array เป็น input แล้วหาผลคูณของค่าทั้งหมดใน array และ return ผลลัพธ์

**ตัวอย่าง**:
`array = [1, 2, 3, 4];`

ผลลัพธ์:
`24`

**หลักการคิด**:
1. เริ่มจากผลลัพธ์ที่ค่าเริ่มต้นคือ 1
2. ทำการคูณค่าทุกตัวใน array
3. Return ผลลัพธ์สุดท้าย

---

#### โจทย์ข้อที่ 12: ลบค่าซ้ำใน array
คำอธิบาย:
ให้รับ array เป็น input และทำการลบค่าที่ซ้ำออก เหลือแค่ค่าไม่ซ้ำใน array แล้ว return array ใหม่

**ตัวอย่าง**:
`array = [1, 2, 2, 3, 3, 4];`

ผลลัพธ์:
`[1, 2, 3, 4]`

**หลักการคิด**:
1. ตรวจสอบและลบค่าที่ซ้ำออกจาก array
2. Return array ที่มีค่าไม่ซ้ำ

---

#### โจทย์ข้อที่ 13: ตรวจสอบว่าค่าใน array อยู่ในช่วงที่กำหนดหรือไม่
คำอธิบาย:
ให้รับ array และค่าที่เป็นช่วง (min, max) จาก input และตรวจสอบว่าค่าทุกค่าของ array อยู่ในช่วงที่กำหนดหรือไม่ แล้ว return ค่า boolean

**ตัวอย่าง**:
`array = [1, 2, 3, 4, 5];`
`min = 1;`
`max = 5;`

ผลลัพธ์:
`true`

**หลักการคิด**:
1. ตรวจสอบแต่ละค่าของ array ว่าอยู่ในช่วง min และ max ที่กำหนดหรือไม่
2. Return true ถ้าทุกค่าอยู่ในช่วง หรือ false ถ้ามีค่าที่ไม่อยู่ในช่วง

---

#### โจทย์ข้อที่ 14: สร้าง array ที่มีค่าตัวเลขคูณด้วย 2
คำอธิบาย:
ให้รับ array ที่มีตัวเลขเป็น input และ return array ใหม่ที่ค่าทุกตัวถูกคูณด้วย 2

**ตัวอย่าง**:
`array = [1, 2, 3];`

ผลลัพธ์:
`[2, 4, 6]`

**หลักการคิด**:
1. ทำการคูณค่าทุกตัวใน array ด้วย 2
2. Return array ใหม่ที่มีค่าถูกคูณแล้ว

---

#### โจทย์ข้อที่ 15: หาค่ากลาง (median) ของ array
คำอธิบาย:
ให้รับ array ที่มีตัวเลขเป็น input และ return ค่ากลาง (median) ของ array

**ตัวอย่าง**:
`array = [1, 2, 3, 4, 5];`

ผลลัพธ์:
`3`

**หลักการคิด**:
1. เรียงค่าใน array จากน้อยไปมาก
2. ถ้าจำนวนสมาชิกใน array เป็นจำนวนคี่ ให้ return ค่ากลาง
3. ถ้าจำนวนสมาชิกใน array เป็นจำนวนคู่ ให้ return ค่ากลางของสองตัวที่อยู่ตรงกลาง


จบไปแล้วกับการอธิบายโจทย์ backend ที่น่าสนใจ หวังว่าทุกท่านจะได้ประโยชน์จากการอธิบายนี้ และสามารถนำไปใช้ประโยชน์ได้ครับ!